// name: Kaleidoscopic Liquid Chrome Core
// author: sirpatch
void main() {
    vec3 color = vec3(0.0);
    vec2 st = aspectUv();
    ...
    // 1. Dynamic Kaleidoscope Space Folding (Slices multiply on bass drops)
    ...
    // Folded coordinate vector
    vec2 kp = vec2(cos(foldedAngle), sin(foldedAngle)) * r;

    // 2. Asymmetric Fluid Domain Warping (Applied to folded kaleidoscope space)
    vec2 fluidUv = kp;
    ...
    // 3. Liquid Core Waveform Surface Tension
    ...
    // 4. Simulated Liquid Chrome Specular Highlight
    ...
    prev *= 0.78; // Maintains pitch-black void
